<template>
  <div class="user-list">
    <el-table class="table-box" :data="userList1" :border="true" :stripe="true">
        ...
        <template v-slot="scope">
          ...
          <el-tooltip :enterable="false" content="分配角色" placement="top">
            <el-button
              ...
              @click="clickSetRoleDialogVisible(scope.row)"
            ></el-button>
          </el-tooltip>
        </template>
      </el-table-column>
...
export default {
  components: { UpdateUserForm, SetRole },
  name: "UserList",
  ...
  methods: {
    ...mapMutations(['setUserRoleDialogVisible', 'setUserInfo']),
    ...
    //点击显示分配权限对话框
    clickSetRoleDialogVisible(userInfo) {
      //保存当前行的用户信息
